...
if number < 0:
    ...
    while abs(number-(ans**4)) >= epsilon and ans**4 <= number:   ###So the while loop should be put after the if check positive?###
        ans += increment
        num_guesses += 1
    
    if abs(number-(ans**4)) < epsilon:   ###Since the increment is mucher smaller than the epsolon, so I think there is no need to worry escape###
        print("ans", ans)
